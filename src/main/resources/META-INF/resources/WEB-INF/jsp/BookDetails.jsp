...
function addLastBorrower(){
	
	var bookid = document.getElementById("bookid").value; 
	
	if(document.getElementById("status").value=="Borrowed"){
		$.ajax({
		     type: "POST",
	         ...
	         success: function(lentbooks) {
	    			...
	    			for(var i=0;i<lentbooks.length;i++){
	    				if(lentbooks[i].bookid==bookid){
	    					document.getElementById("lastBorrower").value=lentbooks[i].borrowerFullName;
	    					document.getElementById("dateOfLending").value=lentbooks[i].dateoflending;
	    					break;
	    				}
	    			}    	
	...
	else if(document.getElementById("status").value=="Returned"){
		
		$.ajax({
		     type: "POST",
		     url: "/Home/GetReturnedBooks",
		     datatype: "json",
	         success: function(returnedbooks) {
	    	  
	    			for(var i=0;i<returnedbooks.length;i++){
	    				if(returnedbooks[i].bookid==bookid){
	    					document.getElementById("lastBorrower").value=returnedbooks[i].borrowerFullName;
	    					document.getElementById("dateOfLending").value=returnedbooks[i].dateoflending;
	    					break;
	    				}
	    			}    	
	...
	else if(document.getElementById("status").value=="Available"){
		
		document.getElementById("lastBorrower").value="Not Yet Borrowed";
...
<div class="container">
  ...
  <form:form id="editform" action="saveBook" modelAttribute="book" method="POST">
   ...
   <form:hidden path="bookid"/>
    ...
    <div class="row">
      ...
      <div class="col-75">
      	 ...
      	 <form:select onmouseout="addauthorid()" id="authorname" path="authorid"  style="height:35px">
    ...
    <div class="row">
     ...
     <div class="col-75">
        <form:input type="text" path="status" readonly="true" />
      </div>
    </div>
   ...
   <div class="row">
      ...
      <div class="col-75">
        <input type="text" id="lastBorrower" readonly="true" >
      </div>
    </div>
   <div id="hid"> 
    <div class="row">
      ...
      <div class="col-75">
        <input type="text" id="dateOfLending" readonly="true">
      </div>
    </div>
   </div>  
    <div class="row">
...
<c:url var="Link" value="/Home/BookTransactions">
